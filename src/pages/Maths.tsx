...
export default function Maths() {
  ...
  const handleActivityComplete = (correct: number, total: number) => {
    ...
    if (selectedLevel) {
      completeLevel("maths", selectedLevel.id);
      addActivity({
        module: "maths",
      ...
      <div className="container mx-auto px-4 py-8">
        {viewMode === "map" && (
          <>
            ...
            <div className="text-center mb-8">
              <h1 className="text-4xl font-bold mb-2">🧮 Magic Maths</h1>
              <p className="text-lg text-muted-foreground">
                Play with numbers and have fun!
              </p>
            </div>

            <LevelMap levels={progress.levelsMaths} onLevelClick={handleLevelClick} />
          </>
        )}
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-3 gap-4 text-center">
                  {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                    <div
                      key={num}
                      className="aspect-square bg-gradient-to-br from-primary/20 to-accent/20 rounded-lg flex items-center justify-center text-4xl font-bold"
                    >
                      {num}
                    </div>
                  ))}
                ...
                <div className="space-y-2">
                  <p className="text-lg">
                    {selectedLevel.category === "addition" && "Addition means putting numbers together! 2 + 3 = 5"}
                    {selectedLevel.category === "subtraction" && "Subtraction means taking away! 5 - 2 = 3"}
                    {selectedLevel.category === "multiplication" && "Multiplication is like adding the same number many times! 3 × 4 = 12"}
                    {selectedLevel.category === "mixed_maths" && "Let's mix different types of problems! You got this!"}
                  </p>
                  <p className="text-lg">
                    Practice makes perfect! Let's try some problems.
                  </p>
                </div>
              </div>
        ...
        {viewMode === "activity" && selectedLevel && (
          <div className="space-y-6">
            ...
            {child.ageGroup === "5-7" ? (
              <MatchGame
                questions={getQuizQuestionsForLevel("maths", selectedLevel.category)}
